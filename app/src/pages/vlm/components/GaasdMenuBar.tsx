...
export function GaasdMenuBar() {
  ...
  return (
    <div
      ref={menuRef}
      className="mllm-topbar h-10 bg-[#6A1B9A] flex items-center px-4 text-sm select-none shrink-0 relative z-[100]"
    >
            ...
            {activeMenu === menuName && (
              <div className="absolute top-full left-0 mt-1 w-48 bg-white text-gray-800 shadow-lg rounded-md border border-gray-200 py-1 flex flex-col z-50">
                {menuData[menuName].map((item) => (
                  <div
                    key={item}
                    className="px-4 py-2 hover:bg-[#F3E5F5] hover:text-[#6A1B9A] cursor-pointer text-xs flex items-center justify-between"
