...
Controller::Controller(std::string configFile)
{
    ...
    double ratioWheelToSteer = config["ratioWheelToSteer"].as<double>();
    dt = config["dt"].as<double>();
    maxDistanceFromTraj = config["maxDistanceFromTraj"].as<double>();
    controlMethod = config["controlMethod"].as<std::string>();
    useGaussianCoordinateSystem = config["useGaussianCoordinateSystem"].as<bool>();
...
ControlCMD Controller::calcControlCMD(State s, const Traj &trajRaw)
{
    ...
    try
    {
        ...
        //  GNSS state check
        // abs(s.rtkMode - 4) > 0.1 && abs(s.rtkMode - 5) > 0.1 && abs(s.rtkMode - 6) > 0.1
        if (s.rtkMode != 4)
        {
            std::cout << BOLDRED << "Warning:  GNSS is not stable\n" << RESET;
            throw std::invalid_argument("GNSS is not stable");
        }
        std::cout << BOLDGREEN << " GNSS status=s.rtkMode: " << s.rtkMode << RESET << std::endl;

        State s_ = s;
        Traj traj = trajRaw;
        // std::cout << "useGaussianCoordinateSystem2: " << useGaussianCoordinateSystem << std::endl;
        // 如果使用高斯坐标系，则将轨迹对象转换为右手坐标系
        //  坐标转化
        if (useGaussianCoordinateSystem)
        {
            // 0201
            transferGauss2RightHandParamStruct transferParam;
            transferGauss2RightHandInputStruct transferInput;
            transferGauss2RightHandOutputStruct transferOutput;
            
            transferParam.trajIn = traj;
            transferParam.sIn = s;
            transferInput.s_ = s_;

            funcTransferGauss2RightHand(transferParam, transferInput, transferOutput);
            traj = transferOutput.trajOut;
            s_ = transferOutput.sOut;
        ...
        if (useInterpolation)
        {
            ...
            std::cout << YELLOW << "calcSplineWithVCourse delay: " << duration.count() << RESET << std::endl;
        }
        ...
        // 0201
        getNearestIndexParamStruct nearestIndexParam;
        getNearestIndexInputStruct nearestIndexInput;
        getNearestIndexOutputStruct nearestIndexOutput;

        nearestIndexParam.state = s_;
        nearestIndexParam.traj = traj;

        funcGetNearestIndex(nearestIndexParam,nearestIndexInput,nearestIndexOutput);
        nearestID = nearestIndexOutput.nearestIndex;
        ...
        if (distance2TrajRaw > maxDistanceFromTraj)
        {
            std::cout << BOLDRED << "ERROR! Traj's NearestPoint distance too far way from vehicle Current State! "
                      << "\n"
                      << "vehicle Current State to NearestPoint distance:" << distance2TrajRaw << RESET << std::endl;
            // throw std::invalid_argument("Traj's NearestPoint distance too far way from vehicle Current State");
        }

        // 计算当前速度
        // 0201
        CalcSpeedParamStruct CalcSpeedParam;
        CalcSpeedParam.s = s_;
        CalcSpeedParam.trajRaw = traj;
        CalcSpeedInputStruct CalcSpeedInput;
        CalcSpeedInput.lonController = lonController;
        CalcSpeedInput.nearestID = nearestID;
        CalcSpeedOutputStruct CalcSpeedOutput;
        CalcSpeed(CalcSpeedParam, CalcSpeedInput,CalcSpeedOutput);
        cmd.speed = CalcSpeedOutput.speedoutput;

        // cmd.speed = lonController.calcSpeed(s_, traj, nearestID);
        // 计算前轮转角

        // CalcSteerParamStruct calcSteerParam;
        // CalcSteerInputStruct calcSteerInput;
        // CalcSteerOutputStruct calcSteerOutput;
        // calcSteerParam.s = s_;
        // calcSteerParam.trajRaw = traj;
        // calcSteerInput.nearestID = nearestID;
        // calcSteerInput.ptrLatController = ptrLatController;
        // funcCalcSteer(calcSteerParam, calcSteerInput, calcSteerOutput);
        // cmd.steer = calcSteerOutput.steeroutput;

        cmd.steer = ptrLatController->calcSteer(s_, traj, nearestID);

        std::cout << BOLDBLUE;
        std::cout << " Speed: " << cmd.speed << std::endl;
        std::cout << " Steer(deg):" << cmd.steer / M_PI * 180 << "\tSteer(rad):" << cmd.steer << std::endl;
        std::cout << RESET;
    }
