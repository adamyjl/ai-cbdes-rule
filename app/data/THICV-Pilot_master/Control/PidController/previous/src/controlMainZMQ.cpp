...
void parseTrajData(Planning::TrajectoryPointVec trajProto, Control::Traj &trajRaw)
{
    ...
    {
        trajRaw.push_back(Control::TrajPoint{tp.x(), tp.y(), tp.theta(), tp.speed(), tp.curvature(), tp.s(),tp.obstacledistance()});
    }
    // std::cout << "@@@@@------parseTrajData: trajRaw[0].obsDIS: " << trajRaw[0].obsDIS << std::endl;
}
...
void serializeControlData(std::string &cmdString, Control::ControlCMD cmd)
{
    ...
    // cmdProto.set_obstacledis(cmd.obsDis);
