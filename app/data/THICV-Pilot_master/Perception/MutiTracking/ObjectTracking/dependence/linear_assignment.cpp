...
TRACHER_MATCHD
linear_assignment::matching_cascade(tracker *distance_metric, tracker::GATED_METRIC_FUNC distance_metric_func,
                                    ...
                                    std::vector<int> detection_indices)
{
    ...
    std::cout << "unmatched_detections.size   " << unmatched_detections.size() << std::endl;
    ...
    for (int level = 0; level < cascade_depth; level++)
    {
        ...
        std::cout << "track_indices_l.size   " << track_indices_l.size() << std::endl;

        if (track_indices_l.size() == 0)
            continue; // Nothing to match at this level.

        TRACHER_MATCHD tmp = min_cost_matching(distance_metric, distance_metric_func, max_distance, tracks,
                                               detections, track_indices_l, unmatched_detections);
        std::cout << "round 1 finished***************************" << std::endl;
...
linear_assignment::min_cost_matching(tracker *distance_metric,
                                     ...
                                     std::vector<Track> &tracks, const DETECTIONS &detections,
                                     std::vector<int> &track_indices, std::vector<int> &detection_indices)
{
    TRACHER_MATCHD res;

    // std::cout << "before detection_indices" << std::endl;

    // for (size_t i = 0; i < detection_indices.size(); i++)
    // {
    //     std::cout << detection_indices[i] << std::endl;
    // }

    // std::cout << "after detection_indices" << std::endl;
    ...
    std::cout << tracks.size() << "   " << detections.size() << "    " << track_indices.size() << "    " << detection_indices.size() << std::endl;

    DYNAMICM cost_matrix =
        (distance_metric->*(distance_metric_func))(tracks, detections, track_indices, detection_indices);
    ...
    cost_matrix = (cost_matrix.array() > max_distance).select(max_distance + 1e-5, cost_matrix);
    std::cout << "in  min_cost_matching "
              << "2222" << std::endl;

    std::cout << cost_matrix.rows() << "   " << cost_matrix.cols() << std::endl;
    ...
    std::cout << "track_indices is " << track_indices.size() << "   detection_indices is   " << detection_indices.size() << std::endl;
    for (int i = 0; i < indices.rows(); i++)
    {
        int row = indices(i, 0);
        int col = indices(i, 1);
...
// DYNAMICM
// linear_assignment::gate_cost_matrix(std::shared_ptr<KalmanFilter> &kf, DYNAMICM &cost_matrix,
//                                     std::vector<Track> &tracks, const DETECTIONS &detections,
