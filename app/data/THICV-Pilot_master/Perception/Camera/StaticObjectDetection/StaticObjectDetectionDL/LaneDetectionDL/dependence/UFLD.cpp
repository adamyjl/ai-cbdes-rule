...
void UFLDLaneDetector::readConfig(string configFile)
{
    YAML::Node config = YAML::LoadFile(configFile);
    engineFile = config["engineFile"].as<string>();
    calibFile = config["calibFile"].as<string>();
}

void UFLDLaneDetector::initTRTmodel(string engineFile)
{
...
void UFLDLaneDetector::initCalib(string calibFile)
{
    // init calibration
...
void UFLDLaneDetector::postProcess()
{
    ...
    for (int k = 0; k < OUTPUT_H; k++)
    {
        ...
        {
            max_ind[k * OUTPUT_W + j] == 100 ? expect[k * OUTPUT_W + j] = 0 : expect[k * OUTPUT_W + j] = expect[k * OUTPUT_W + j];
        }
    }
    ...
    for (int k = 0; k < OUTPUT_H; k++)
    {
        for (int ll = 0; ll < i_ind.size(); ll++)
        {
            ...
            {
                lanes[i_ind[ll]][k][0] = int(expect[OUTPUT_W * k + i_ind[ll]] * col_sample_w) - 1;
                lanes[i_ind[ll]][k][1] = int(tusimple_row_anchor[OUTPUT_H - 1 - k]) - 1;
            }
        }
    }
}
...
bool UFLDLaneDetector::display()
{
    for (int k = 0; k < OUTPUT_H; k++)
    {
        for (int l = 0; l < OUTPUT_W; l++)
        {
            if (lanes[l][k] > 0)
            {
                cv::Point pp = {lanes[l][k][0], lanes[l][k][1]};
...
void UFLDLaneDetector::toProto()
{
    for (int i = 0; i < OUTPUT_W; i++)
    {
        lane::Lane *lane_tmp = laneResult.add_lane();
        for (int j = 0; j < OUTPUT_H; i++)
        {
            lane::Point *point_tmp = lane_tmp->add_point();
            point_tmp->set_x(lanes[i][j][0]);
            point_tmp->set_y(lanes[i][j][1]);
        }
    }
}

void UFLDLaneDetector::run()
{
