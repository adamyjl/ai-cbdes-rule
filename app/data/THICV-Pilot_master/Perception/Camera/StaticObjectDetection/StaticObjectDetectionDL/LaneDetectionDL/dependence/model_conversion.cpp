...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder,  DataType dt) {
    ...
    ITensor* data = network->addInput(INPUT_BLOB_NAME, dt, Dims4{1, INPUT_C, INPUT_H, INPUT_W });
    assert(data);
    ...
    auto matrixMultLayer0 = network->addMatrixMultiply(*permute0->getOutput(0), 
            nvinfer1::MatrixOperation::kNONE, *fcwts0->getOutput(0), nvinfer1::MatrixOperation::kTRANSPOSE);
    ...
    auto matrixMultLayer1 = network->addMatrixMultiply(*relu->getOutput(0), 
            nvinfer1::MatrixOperation::kNONE, *fcwts1->getOutput(0), nvinfer1::MatrixOperation::kTRANSPOSE);
